...
// Types for photo suggestions (matching server-side types)
...
export function AIPhotoReview({ contact, open, onOpenChange }: AIPhotoReviewProps) {
  ...
  const downloadMutation = useMutation({
    mutationFn: async (suggestion: PhotoSuggestion): Promise<unknown> => {
      if (!contact) {
  ...
  const searchForPhotos = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to search for photos.'
      toast({
        title: 'Search failed',
  ...
  }, [open, suggestions.length, contact, searchForPhotos]);
        ...
        <ScrollArea className='max-h-[400px] pr-4'>
          {isSearching ? (
            ...
            <div className='text-center py-8'>
              ...
              <Button variant='outline' size='sm' className='mt-4' onClick={searchForPhotos}>
                Search Again
              </Button>
            </div>
          ) : (
                            ...
                            <div className='flex items-center gap-2'>
                              ...
                              {suggestion.metadata?.sourceUrl && (
                                <Button
                                  ...
                                  variant='ghost'
                                  onClick={() =>
                                    window.open(suggestion.metadata?.sourceUrl, 'blank')
                                  }
                                >
