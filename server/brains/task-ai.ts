...
import type { Task, AiSuggestion, Contact, Tag } from '../../shared/schema.js';
import type { AttendanceRecord } from '../types/external-apis.js';
import type {
  AttendanceAnalysis,
  BulkAction,
  TaskAnalysis,
...
function isBulkAction(action: unknown): action is BulkAction {
  ...
  return (
    ...
    ['bulk_timeline_update', 'bulk_photo_update', 'bulk_contact_update'].includes(actionObj.type)
  );
}
...
export class TaskAIService {
  ...
  async processAttendanceCSV(
    ...
    try {
      ...
      const job = await storage.createDataProcessingJob({
        userId,
        jobType: 'attendance_csv' as const,
      ...
      if (!job) {
        throw new Error('Failed to create data processing job');
      }
  ...
  async processNewPhotos(
    ...
    try {
      const job = await storage.createDataProcessingJob({
        userId,
        jobType: 'photo_gdpr' as const,
        sourceType: 'ai_enrichment' as const,
  ...
  async delegateTaskToAI(
    ...
    await storage.createTaskActivity({
      ...
      actorId: userId,
      actionType: 'delegated_to_ai' as const,
  ...
  private async processAIDelegatedTask(task: Task): Promise<void> {
    try {
      ...
      const contactIds = (task.assignedContactIds as string[]) ?? [];
      const contactPromises = contactIds.map(async (id) => {
        try {
          return await storage.getContact(id);
        } catch (error) {
      ...
      const validContacts = contacts.filter(
        (contact): contact is Contact & { tags: Tag[] } => contact !== undefined
      );
      ...
      const newStatus = subtasks.length > 0 ? 'in_progress' : 'completed';
      ...
      await storage.createTaskActivity({
        ...
        actionType: 'ai_processed' as const,
...
Return JSON with: {
  ...
  "estimatedTime": "time estimate in minutes"
    ...
    try {
      const response = await aiService.processGenericPrompt(prompt, task.userId);
