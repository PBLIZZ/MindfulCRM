...
import { llmConcurrencyController } from './llm-concurrency-controller.js';
import { llmCostTracker } from './llm-cost-tracker.js';
...
export class PhotoEnrichmentService {
  ...
  /**
   * Extract social media handles from contact information using LLM with cost tracking
   */
  async extractSocialMediaHandles(
    contact: ContactInfo,
    userId?: string
  ): Promise<SocialMediaHandles> {
    const prompt = `
...
Only include URLs that you can reasonably infer from the provided information. Use null for platforms where you cannot make a reasonable guess.`;
    ...
    try {
      // Use concurrency controller if userId is provided
      if (userId) {
        const operation = () => aiService.processWithKimi(prompt);
        const response = await llmConcurrencyController.execute(operation, {
          userId,
        ...
        const response = await aiService.processWithKimi(prompt);
        const handles = JSON.parse(response) as SocialMediaHandles;
        return handles;
      }
    } catch (error) {
  ...
  async findPhotoSuggestions(contact: ContactInfo): Promise<PhotoSuggestion[]> {
    ...
    try {
      // First, extract social media handles using LLM (pass userId for tracking if available)
      const socialHandles = await this.extractSocialMediaHandles(contact, 'system');
  ...
  /**
   * Batch process photo enrichment for all consented contacts with concurrency control
   */
  async batchEnrichPhotos(userId: string): Promise<BatchEnrichmentResult> {
    try {
      ...
      const allContacts = await storage.contacts.getByUserId(userId);
      const consentedContacts = allContacts.filter(
        (contact) => contact.hasGdprConsent && !contact.avatarUrl
      );

      console.log(`Starting batch photo enrichment for ${consentedContacts.length} contacts`);

      // Get cost recommendation for this batch
      const costRecommendation = llmCostTracker.getModelRecommendation(
        userId,
        'photo_enrichment',
        consentedContacts.length * 200 // Estimate 200 tokens per contact for social media extraction
      );

      console.log(
        `Using model: ${costRecommendation.recommendedModel} (${costRecommendation.reason})`
      );
      console.log(`Estimated batch cost: $${costRecommendation.estimatedCost.toFixed(4)}`);

      // Create operations for concurrent processing
      const operations = consentedContacts.map((contact) => ({
        operation: () => this.processContactPhotoEnrichment(contact, userId),
        userId,
        model: 'photo_enrichment', // Not actually an LLM model, but used for tracking
        priority: 'low' as const, // Photo enrichment is lower priority
      }));

      // Execute with controlled concurrency
      const results = await llmConcurrencyController.executeBatch(operations, {
        batchSize: 5, // Process 5 contacts at a time
        delayBetweenBatches: 1000, // 1 second delay between batches to respect external API limits
      });
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const contact = consentedContacts[i];

        if (result.success) {
          batchResult.success++;
          console.log(`✓ Enriched photo for ${contact.name}`);
        } else {
          batchResult.errors.push(`${contact.name}: ${result.error}`);
        }
      }

      console.log(
        `Batch enrichment completed: ${batchResult.success}/${batchResult.processed} successful`
      );
      return batchResult;
    } catch (error) {
      throw new Error(
  ...
  /**
   * Process photo enrichment for a single contact
   */
  private async processContactPhotoEnrichment(
    contact: ContactInfo,
    _userId: string
  ): Promise<void> {
   ...
   */
  async enrichSingleContact(contactId: string): Promise<EnrichmentResult> {
    try {
